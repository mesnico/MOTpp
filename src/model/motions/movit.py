...
from src.guofeats.motion_representation import recover_rot_pos
import torch
...
class MoViT(nn.Module):
	...
	def __init__(self,
		...
		self.skel_parts_ids = skeleton_parts_ids[dataset] if body_repres == 'body-parts' else None
		if body_repres == 'full':
		...
		self.pose_proj = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.embed_dims, latent_dim * 2 if vae else latent_dim),
        )
		self.vae = vae
	...
	def forward(self, x_dict):
		...
		x = recover_rot_pos(x, get_velocities=True)
