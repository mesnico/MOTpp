...
class DividedTemporalAttentionWithPreNorm(nn.Module):
	...
	def forward(self, query, key=None, value=None, residual=None, return_attention=False, padding_mask=None, attention_weights=None, **kwargs):
		...
		cls_token = query[:, 0:self.num_cls_tokens, :]
		if self.use_cls_token:
			residual = query
			query = query[:, self.num_cls_tokens:, :]
		else:
			query = query[:, self.num_cls_tokens:, :]
			residual = query
		...
		if self.use_cls_token:
			...
			cls_token = rearrange(cls_token, 'b p n d -> (b p) n d')
		...
		# Post-Process
		if self.use_cls_token:
			cls_token, attn_out = attn_out[:, :self.num_cls_tokens, :], attn_out[:, self.num_cls_tokens:, :]
			cls_token = rearrange(cls_token, '(b p) n d -> b p n d', b=b)
			cls_token = reduce(cls_token, 'b p n d -> b n d', 'mean')
...
class DividedSpatialAttentionWithPreNorm(nn.Module):
	...
	def forward(self, query, key=None, value=None, residual=None, return_attention=False, padding_mask=None, attention_weights=None, **kwargs):
		...
		if self.use_cls_token:
			...
			cls_token = rearrange(cls_token, 'b t n d -> (b t) n d')
			query = torch.cat((cls_token, query), 1)
		...
		# Post-Process
		if self.use_cls_token:
			cls_token, attn_out = attn_out[:, 0:self.num_cls_tokens, :], attn_out[:, self.num_cls_tokens:, :]
			cls_token = rearrange(cls_token, '(b t) n d -> b t n d', b=b)
			cls_token = reduce(cls_token, 'b t n d -> b n d', 'mean')
...
class RecurrentTransformerContainer(nn.Module):
	...
	def __init__(self, 
				 ...
				 act_layer=nn.GELU,
				 num_layers=2,
				 aggregate_multiple_layers=None):
		super().__init__()
		#self.layers = nn.ModuleList([])
		self.num_transformer_layers = num_transformer_layers
		self.aggregate_multiple_layers = aggregate_multiple_layers
		
		# dpr = np.linspace(0, drop_path_rate, num_transformer_layers)
		self.transf_layer = BasicTransformerBlock(
			embed_dims=embed_dims,
			num_heads=num_heads,
			num_frames=num_frames,
			hidden_channels=hidden_channels,
		...
		if self.aggregate_multiple_layers == 'transformer':
			aggr_transformer_layer = nn.TransformerEncoderLayer(
				d_model=embed_dims, 
				nhead=1,
				dim_feedforward=embed_dims,
				dropout=drop_path_rate)
			self.aggr_transformer = nn.TransformerEncoder(aggr_transformer_layer, num_layers=1)
			self.aggr_pos_embedding = nn.Parameter(torch.zeros(1, num_transformer_layers, embed_dims))
		elif self.aggregate_multiple_layers == 'gru':
			self.aggr_gru = nn.GRU(embed_dims, embed_dims, batch_first=True)
		
	def forward(self, x, temporal_mask=None, skeleton_mask=None, return_attention=False):
		layer_features = []
		for layer_idx in range(self.num_transformer_layers):
			if layer_idx >= self.num_transformer_layers-1 and return_attention:
				x = self.transf_layer(x, temporal_mask, skeleton_mask, return_attention=True)
			else:
				x = self.transf_layer(x, temporal_mask, skeleton_mask)
			layer_features.append(x[:, 0])

		if not return_attention and self.aggregate_multiple_layers is not None:
			x = torch.stack(layer_features, dim=1)
			if self.aggregate_multiple_layers == 'mean':
				x = x.mean(dim=1)
			elif self.aggregate_multiple_layers == 'transformer':
				# add positional encoding
				x = x + self.aggr_pos_embedding
				# use a transformer to aggregate the features
				x = self.aggr_transformer(x.permute(1, 0, 2))
				x = x[0]
			elif self.aggregate_multiple_layers == 'gru':
				...
				x = x[:, -1]
			x = x.unsqueeze(1)	# to maintain compatibilty with non-aggregated features
		return x
