...
from src.guofeats.motion_representation import recover_rot_pos_bis
import torch
import torch.nn as nn

from .transformer_core import TransformerContainer, RecurrentTransformerContainer, get_sine_cosine_pos_emb
from .skeleton import skeleton_parts_ids_v2
from .gcnutils import Graph

class MLP(nn.Module):
	def __init__(self, in_features, out_features, linear=True, hidden_features=None):
		super().__init__()
...
class MoViT(nn.Module):
	...
	def __init__(self,
				 ...
				 use_learnable_pos_emb_temporal=False,
				 use_learnable_pos_emb_spatial=True,
		...
		assert return_cls_token, 'Only return_cls_token=True is supported for now!'
		
		self.nfeats = 263 	# humanml
		...
		self.use_learnable_pos_emb_spatial = use_learnable_pos_emb_spatial
		self.use_learnable_pos_emb_temporal = use_learnable_pos_emb_temporal
		self.return_cls_token = return_cls_token
		self.body_repres = body_repres
		self.uniform_sample = uniform_sample

		transformer_class = RecurrentTransformerContainer if recurrent else TransformerContainer

		self.skel_parts_ids = skeleton_parts_ids_v2[dataset] if body_repres == 'body-parts' else None
		if body_repres == 'full':
			num_patches = 21 if dataset == 'kit' else 22
		else:
			num_patches = 7

		num_clss = 2 if vae else 1
		
		if self.attention_type == 'divided_space_time':
			# Divided Space Time Attention - Model 3
			operator_order = ['time_attn','space_attn','ffn']
			container = transformer_class(
				num_transformer_layers=num_transformer_layers,
				embed_dims=embed_dims,
				num_heads=num_heads,
				num_frames=num_frames,
				norm_layer=norm_layer,
				hidden_channels=ff_dims,
				operator_order=operator_order,
				num_cls_tokens=num_clss,
				**transformer_kwargs)
			...
			spatial_transformer = transformer_class(
				num_transformer_layers=num_transformer_layers,
				embed_dims=embed_dims,
				num_heads=num_heads,
				num_frames=num_frames,
			...
			temporal_transformer = transformer_class(
				num_transformer_layers=num_transformer_layers,
				embed_dims=embed_dims,
				num_heads=num_heads,
				num_frames=num_frames,
		...
		self.cls_token = nn.Parameter(torch.zeros(1, num_clss, embed_dims))
		# whether to add one cls_token in temporal pos_enb
		if attention_type == 'fact_encoder':
			num_frames = num_frames + num_clss
			num_patches = num_patches + num_clss
		...
		if use_learnable_pos_emb_temporal:
		...
		if use_learnable_pos_emb_spatial:
		...
		f = 6 if data_rep == 'cont_6d' else 9 if data_rep == 'cont_6d_plus_rifke' else 12
		if body_repres == 'body-parts':
			self.layer1_rarm_enc = MLP(f * len(self.skel_parts_ids['right_arm']), embed_dims)
			self.layer1_larm_enc = MLP(f * len(self.skel_parts_ids['left_arm']), embed_dims)
			self.layer1_rleg_enc = MLP(f * len(self.skel_parts_ids['right_leg']), embed_dims)
			self.layer1_lleg_enc = MLP(f * len(self.skel_parts_ids['left_leg']), embed_dims)
			self.layer1_torso_enc = MLP(f * len(self.skel_parts_ids['mid_body']), embed_dims)
			self.layer1_root_enc = MLP(f * len(self.skel_parts_ids['root']), embed_dims)
			self.layer1_feet_enc = MLP(f * len(self.skel_parts_ids['feet']), embed_dims)
		...
		self.pose_proj = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.embed_dims, latent_dim),
        )
		self.vae = vae
	...
	def prepare_tokens(self, x):
		...
		if self.use_cls_token_temporal:
			if self.use_learnable_pos_emb_spatial:
				x = x + self.pos_embed
			else:
		...
		if self.attention_type != 'fact_encoder':
			num_clss = cls_tokens.shape[1]
			cls_tokens = x[:b, :num_clss, :]
			if self.use_cls_token_temporal:
				x = rearrange(x[:, num_clss:, :], '(b t) p d -> (b p) t d', b=b)
				cls_tokens = repeat(cls_tokens,
									'b ... -> (repeat b) ...',
									repeat=x.shape[0]//b)
				x = torch.cat((cls_tokens, x), dim=1)
				if self.use_learnable_pos_emb_temporal:
					x = x + self.time_embed
				else:
					x = x + self.time_embed.type_as(x).detach()
				cls_tokens = x[:b, :num_clss, :]
				x = rearrange(x[:, num_clss:, :], '(b p) t d -> b (p t) d', b=b)
				x = torch.cat((cls_tokens, x), dim=1)
			else:
				x = rearrange(x[:, num_clss:, :], '(b t) p d -> (b p) t d', b=b)
				if self.use_learnable_pos_emb_temporal:
	...
	def forward(self, x_dict):
		...
		# Pad dimension 1 of x to self.num_frames with zeros (to match time_embed)
		x = torch.nn.functional.pad(x, (0, 0, 0, self.num_frames - x.size(1)), value=0)

		# Transform to joints-based representation
		x = recover_rot_pos_bis(x)

		N, T, V, C = x.size()

		# temporal uniform subsample and mask construction
		x = x.permute(0, 3, 2, 1).contiguous()  # N x C x V x T
		x_new = []
		temporal_mask = torch.zeros(N, self.num_frames).bool()
		temporal_mask = temporal_mask.to(x.device)
		for i, (L, m) in enumerate(zip(lengths, temporal_mask)):
			t = x[i:i+1]
		...
		x = x.permute(0, 3, 2, 1).contiguous()  # N x T x V x C

		if not self.use_time_padding_mask:
			temporal_mask = None
		# bs = x.shape[0]
		...
		if self.body_repres == 'body-parts':
			...
			root = x[..., self.skel_parts_ids['root'], :].view(b, t, -1)
			feet = x[..., self.skel_parts_ids['feet'], :].view(b, t, -1)
			...
			root_layer1 = self.layer1_root_enc(root)
			feet_layer1 = self.layer1_feet_enc(feet)
		...
		num_clss = self.cls_token.shape[1]
		...
		if self.attention_type != 'fact_encoder':
			...
			x = spatial_transformer(x, skeleton_mask=skeleton_att_mask)
			
			# Add Time Embedding
			cls_tokens = x[:b, 0:num_clss, :]
			x = rearrange(x[:, num_clss:, :], '(b t) p d -> b t p d', b=b)
			...
			x = temporal_transformer(x, temporal_mask=temporal_mask)
