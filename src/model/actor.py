...
from src.guofeats.motion_representation import recover_rot_pos, recover_rot_pos_bis
...
class ACTORStyleEncoder(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        activation: str = "gelu",
        debug_encode_decode_joints = False
    ) -> None:
        super().__init__()

        self.nfeats = nfeats
        self.projection = nn.Linear(nfeats if not debug_encode_decode_joints else (22+1)*12, latent_dim)

        self.vae = vae
        self.debug_encode_decode_joints = debug_encode_decode_joints
    ...
    def forward(self, x_dict: Dict) -> Tensor:
        ...
        if 'text' not in x_dict and self.debug_encode_decode_joints:
