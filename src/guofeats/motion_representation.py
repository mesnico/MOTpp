...
def recover_rot_pos(data, get_velocities=True):
    """
    ...
    if get_velocities:
        ...
        vels = data[..., start_indx:end_indx].view(bs, seq_len, joints_num, 3)
        rot_pos = torch.cat([rot_pos, vels], dim=-1) # bs x seqlen x num_joints x 12
        
    return rot_pos


def recover_rot_pos_bis(data):
    """
    Same as recover_rot but include in the final features also the rifke positions (6+3 features for each joint)
    """
    dims_per_joint = 12
    ...
    # get feet data
    start_indx = 1 + 2 + 1 + (joints_num - 1) * 3 + (joints_num - 1) * 6 + joints_num * 3
    feet_data = data[..., start_indx:]
    # pad to n dimensions
    # feet_data = torch.cat([feet_data, torch.zeros(bs, seq_len, dims - feet_data.shape[-1]).to(feet_data.device)], dim=-1)
    ...
    cont6d_params = data[..., start_indx:end_indx]
    # cont6d_params = torch.cat([r_rot_cont6d, cont6d_params], dim=-1)
    cont6d_params = cont6d_params.view(bs, seq_len, joints_num - 1, 6)
    ...
    pos_params = data[..., start_indx:end_indx]
    # pos_params = torch.cat([r_pos, pos_params], dim=-1)
    pos_params = pos_params.view(bs, seq_len, joints_num - 1, 3)
    ...
    end_indx = start_indx + joints_num * 3
    vels = data[..., start_indx:end_indx].view(bs, seq_len, joints_num, 3)
    root_vels, vels = vels[:, :, 0:1, :], vels[:, :, 1:, :]
    rot_pos = torch.cat([rot_pos, vels], dim=-1) # bs x seqlen x num_joints x 12

    # root data
    root_data = torch.cat([root_data.unsqueeze(2), root_vels], dim=-1)
    # pad to 12 dimensions
    root_data = torch.cat([root_data, torch.zeros(bs, seq_len, 1, dims_per_joint - root_data.shape[-1]).to(root_data.device)], dim=-1)

    # feet data
    feet_data = feet_data.unsqueeze(2)
    # pad to 12 dimensions
    feet_data = torch.cat([feet_data, torch.zeros(bs, seq_len, 1, dims_per_joint - feet_data.shape[-1]).to(feet_data.device)], dim=-1)

    # concat over the joints dimension
    rot_pos = torch.cat([root_data, rot_pos, feet_data], dim=2)
