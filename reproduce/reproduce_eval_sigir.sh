...
CKPT=latest #best-rsum # best-rsum-t2m # best-rsum # latest
SKIP_DONE=true
            ...
            for LOSS in ${LOSSES[@]}; do
                for DATASET in ${DATASETS[@]}; do
                    for DATA_REP in ${DATA_REPS[@]}; do
                        EXP_PATH=${EXP_ROOT}/lr=${LR}/model=t2m/data=${DATASET}/length_regressor=None/loss=${LOSS}/motion=${MOTION_MODEL}/text=${TEXT_MODEL}/data_rep=${DATA_REP}/space-dim=${SPACE_DIM}/rep=${REP}
                        if [ ! -d "$EXP_PATH" ]; then
