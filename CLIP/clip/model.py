...
class PromptTokensModule(nn.Module):
    def __init__(self, num_prompt_tokens: int, width: int, inner_width: int = 128):
        super().__init__()
        self.prompt_tokens = nn.Parameter(torch.randn(1, num_prompt_tokens, inner_width))
    ...
    def forward(self, x: torch.Tensor):
        projected = self.mlp(self.prompt_tokens)
        return torch.cat([projected.expand(x.shape[0], -1, -1), x], dim=1)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if num_shallow_text_prompt_tokens != 0:
            self.prompt_tokens_module = PromptTokensModule(num_shallow_text_prompt_tokens, transformer_width)
        else:
            self.prompt_tokens_module = None
    ...
    def build_attention_mask(self):
        ...
        mask.triu_(1)
        # mask.triu_(1 + self.num_shallow_text_prompt_tokens)  # zero out the lower diagonal, making sure all prompt tokens are attended always
        return mask
    ...
    def encode_text(self, text, features=None, normal_beh=True):
        ...
        if self.prompt_tokens_module is not None:
            x = self.prompt_tokens_module(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        cls1 = x[torch.arange(x.shape[0]), cls_idx] # @ self.text_projection
        cls2 = x[torch.arange(x.shape[0]), cls_idx - 1]
        cls = torch.stack([cls1, cls2], dim=1)
        return cls
